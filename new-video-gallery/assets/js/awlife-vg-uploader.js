		...
		awlife_grid_gallery = {
						...
						file_frame.on(
							...
							function() {
								...
								for (var i = 0; i < length; i++) {
									awlife_grid_gallery.get_thumbnail( images[i]['id'], '', vg_add_images_nonce);
								}
							}
						);
						file_frame.open();
			...
			get_thumbnail: function(id, cb, vg_add_images_nonce) {
				...
				jQuery.post(
					...
					data,
					function(response) {
						awlife_grid_gallery.ul.append( response );
						cb();
					}
				);
			}
		};
		awlife_grid_gallery.init();
	}
);
